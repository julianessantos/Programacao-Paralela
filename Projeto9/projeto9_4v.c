...
// Versão 4: N listas com locks explícitos
void versao4(int qtd_listas) {
    if (qtd_listas > MAX_LISTAS) qtd_listas = MAX_LISTAS;
    Node* listas[MAX_LISTAS] = { NULL };
    omp_lock_t locks[MAX_LISTAS];
    ...
    printf("Tempo de execucao (Versao 4): %.2f ms\n\n", tempo);
...
int main() {
    printf("--- Versao 4 ---\n");
    versao4(10);  // Exemplo com 5 listas
    return 0;
}
